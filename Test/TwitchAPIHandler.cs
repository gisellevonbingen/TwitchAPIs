...
namespace TwitchAPI.Test
{
    public class TwitchAPIHandler
    {
        ...
        public bool Create()
        {
            ...
            var crawler = this.Crawler = new TwitchCrawler();
            ...
            if (responseURI != null)
            {
                var oAuth = this.OAuthAuthorization = crawler.GetOAuthAuthorization(responseURI, authRequest);
        ...
        private OAuthRequest CreateOAuthRequest(UserAbstract user)
        {
            ...
            var input = user.QueryInput("Enter RequestType", list.Select(req => req.GetType().Name).ToArray());
